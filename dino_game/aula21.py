...
sprite_sheet = pygame.image.load(os.path.join(diretorio_imagens, 'Dino_Spritesheets.png')).convert_alpha()
...
class Dino(pygame.sprite.Sprite):
    def __init__(self):
        ...
        for frames in range(3):
            img = sprite_sheet.subsurface((frames * 32, 0), (32, 32))
        ...
        self.posicao_y_inicial = altura - 70 - 96//2
        self.rect.topleft = (100, self.posicao_y_inicial) 
        self.pulo = False
    ...
    def update(self):
        ...
        if self.pulo == True:
            if self.rect.y <= self.posicao_y_inicial - 150:
                self.pulo = False
            self.rect.y -= 15
...
class DinoVoador(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.imagens_dinossauro = []
        for frames_dinoVoador in range(3,5):
            img = sprite_sheet.subsurface((frames_dinoVoador*32, 0), (32,32))
            img = pygame.transform.scale(img, (32*3, 32*3))
            self.imagens_dinossauro.append(img)
...
for geracao_nuvem in range(4):
    nuvem = Nuvens()
    todas_as_sprites.add(nuvem)

for geracao_chao in range(largura*2//64):
    chao = Chao(geracao_chao)
